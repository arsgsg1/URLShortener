        ...
        urlButton.addEventListener('click', function(){
            httpRequest.onload = function() {
                if (httpRequest.readyState == XMLHttpRequest.DONE && httpRequest.status == 200 ) {
                    ...
                    result.value = urlShorten.convertedUrl;
                }
                else if(httpRequest.readyState == XMLHttpRequest.DONE && httpRequest.status == 400){
                    let errMessage = JSON.parse(httpRequest.responseText);
                    alert(errMessage.errors[0].reason);
                }
            }
            let data = {
